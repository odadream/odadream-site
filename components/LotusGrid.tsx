...
import { 
    Undo2, Maximize2, ChevronUp, GripHorizontal, ArrowUp, ExternalLink,
    Film, AudioLines, Image as ImageIcon, Layers, Zap, FileText, Disc, Plus
} from 'lucide-react';
...
const CornerBrackets = ({ show, accent }: { show: boolean; accent: boolean }) => {
    ...
    const opacityClass = accent 
        ? 'opacity-100' 
        : (show ? 'opacity-30 group-hover:opacity-100 transition-opacity duration-300' : 'opacity-0');
        
    const length = "w-1.5 h-1.5 md:w-2 md:h-2"; 

    return (
        <div className={`absolute inset-0 pointer-events-none z-30 ${opacityClass}`}>
            <div className={`absolute top-0 left-0 ${length} border-t border-l transition-colors duration-300 ${borderColor} ${glowClass}`} />
            <div className={`absolute top-0 right-0 ${length} border-t border-r transition-colors duration-300 ${borderColor} ${glowClass}`} />
            <div className={`absolute bottom-0 left-0 ${length} border-b border-l transition-colors duration-300 ${borderColor} ${glowClass}`} />
            <div className={`absolute bottom-0 right-0 ${length} border-b border-r transition-colors duration-300 ${borderColor} ${glowClass}`} />
        </div>
    );
};
...
const CSSFallbackPattern = React.memo(({ id, isCenter }: { id: string, isCenter: boolean }) => {
    ...
    return (
        <div className="absolute inset-0 w-full h-full overflow-hidden bg-[#080808]">
            ...
            />
            <div className={`absolute w-[150%] h-[150%] -left-[25%] -top-[25%] opacity-15 bg-[radial-gradient(circle_at_center,theme('colors.accent.DEFAULT'),transparent)] blur-3xl`} />
        </div>
    );
});
...
const GridCell = React.memo(({
    ...
    const opacityClass = cell.isCenter 
        ? 'opacity-100' 
        : (isVisualNode ? 'opacity-80 hover:opacity-100' : 'opacity-40 hover:opacity-100'); 
    ...
    let TypeIcon;
    switch (cell.type) {
        case 'hub': TypeIcon = Layers; break;
        case 'action': TypeIcon = Zap; break;
        case 'media':
            switch (mediaType) {
                case 'video': TypeIcon = Film; break;
                case 'audio': TypeIcon = AudioLines; break;
                case 'image': TypeIcon = ImageIcon; break;
                default: TypeIcon = ImageIcon;
            }
            break;
        default: TypeIcon = FileText;
    }

    const showChrome = cell.isCenter || isHovered;
    const contentOpacity = cell.isCenter || isHovered ? 'opacity-100' : 'opacity-70';
    ...
    return (
        ...
        >
            ...
            {/* BACKGROUND LAYER */}
            <div className={`absolute inset-[2px] z-0 overflow-hidden bg-black transition-all duration-700 ease-out ${opacityClass} ${grayscaleClass}`}>
            ...
            {/* FOREGROUND CONTENT */}
            <div className={`relative z-20 flex flex-col items-center justify-between w-full h-full p-2 md:p-4 text-center pointer-events-none transition-opacity duration-300 ${contentOpacity}`}>
...
export const LotusGrid: React.FC<LotusGridProps> = ({ 
  ...
  return (
    <MotionDiv 
        ...
        initial={false}
        animate={isLandscape ? "desktop" : (isCollapsed ? "mobileCollapsed" : "mobileExpanded")}
        variants={variants}
        ...
        >
             ...
             <div className="md:hidden landscape:hidden text-zinc-500 flex items-center justify-center w-8 h-8 rounded-full active:bg-zinc-800 transition-colors relative z-20">
                <MotionDiv 
                   ...
                   whileTap={{ scale: 1.2 }}
                >
                    {isCollapsed ? <ChevronUp className="w-4 h-4" /> : <GripHorizontal className="w-4 h-4 opacity-70" />}
                </MotionDiv>
             </div>
        </div>
