...
export const THEME = {
  ...
  interactive: {
    linkInternal:
      "inline-flex items-center gap-1 border-b border-accent/20 text-accent hover:brightness-125 hover:border-accent hover:drop-shadow-laser transition-all pb-px no-underline leading-[1.1]",
    linkMedia:
      "group inline-flex items-center justify-center gap-2 px-3 py-1.5 mx-1.5 my-1 text-[10px] font-bold tracking-[0.15em] uppercase text-txt-main bg-overlay/5 border border-border hover:text-txt-muted hover:border-overlay/20 hover:bg-overlay/10 transition-all rounded-[1px] select-none whitespace-nowrap backdrop-blur-[1px] [&>span]:leading-[1.1] [&>svg]:flex-shrink-0",
  },
  ...
  lotus: {
    ...
    image: {
      ...
      visual:
        "opacity-60 group-hover:opacity-90 grayscale group-hover:grayscale-0 transition-all duration-700 ease-out",
      // Non-visual nodes: subtle grayscale, more opacity on hover
      default:
        "opacity-30 group-hover:opacity-80 grayscale group-hover:grayscale-0 transition-all duration-700 ease-out",
    },
    ...
    glow: {
      ...
      centerGlow:
        "shadow-[0_0_60px_-10px_rgb(var(--color-accent)/0.25),0_0_30px_-5px_rgb(var(--color-accent)/0.15)]",
      // Subtle ambient glow on neighboring cells when center is active
      neighborGlow:
        "group-has-[:is(.cell-active)]:shadow-[0_0_20px_-5px_rgb(var(--color-accent)/0.1)]",
    },
  },
};
