...
export const TextPanel: React.FC = () => {
  ...
  const markdownComponents: Components = useMemo(
    () => ({
      ...
      blockquote: ({ children }) => (
        <blockquote className="relative border-l-2 border-border-dim pl-4 py-3 my-8 text-txt-muted text-sm italic [&>p]:!my-0 [&>p]:leading-snug">
          {children}
        </blockquote>
      ),
      ul: ({ children }) => (
        <ul className="list-disc list-outside pl-6 space-y-1 my-4 marker:text-txt-dim">
          {children}
        </ul>
      ),
      ol: ({ children }) => (
        <ol className="list-decimal list-outside pl-6 space-y-1 my-4 marker:text-txt-dim marker:font-mono marker:text-xs text-txt-muted">
          {children}
        </ol>
      ),
      li: ({ children }) => (
        <li className="text-txt-muted text-left leading-relaxed pl-1">
          {children}
        </li>
      ),
      a: ({ href, children }) => {
        if (href?.includes("?internal=true")) {
          ...
          return (
            <button
              onClick={() => jumpToId(id)}
              className={THEME.interactive.linkInternal}
            >
              {children}
            </button>
          );
      ...
      code: ({ children }) => (
        <code className="font-mono text-[0.8em] text-txt-muted bg-surface/50 px-1 rounded border border-border-dim mx-0.5">
          {children}
        </code>
      ),
      hr: () => <hr className="border-t border-border-dim my-8 opacity-50" />,
      strong: ({ children }) => (
        <strong className="text-hud font-bold drop-shadow-projector">
          {children}
        </strong>
      ),
      em: ({ children }) => (
        <em className="italic text-txt-muted font-serif tracking-wide">
          {children}
        </em>
      ),
      p: ({ children }) => (
        <p className="mb-4 text-left last:mb-0 leading-relaxed">{children}</p>
      ),
    }),
    [jumpToId, lang],
  );
  ...
  return (
    <div className={`relative flex-1 min-h-0 flex flex-col w-full`}>
      <div
        ref={scrollRef}
        className={`${THEME.panel.scrollableText} ${THEME.layout.paddingX} ${THEME.layout.paddingY}`}
      >
            ...
            <div className={THEME.typography.contentFooter}>
              <div className="flex flex-wrap justify-between gap-y-8 gap-x-12 items-start w-full">
                <div className="flex flex-col gap-2 min-w-[200px] flex-1">
                  <span
                    className={`${THEME.typography.meta} text-txt-dim select-none`}
                  >
                    KEYWORDS
                  </span>
                  {currentNode.tags && currentNode.tags.length > 0 ? (
                    <div className="flex flex-wrap gap-x-3 gap-y-2 leading-relaxed">
                      {currentNode.tags.map((tag) => (
                        <span
                          key={tag}
                          className={`${THEME.typography.meta} text-txt-muted`}
                        >
                          <span className="text-txt-dim mr-px">#</span>
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <span
                      className={`${THEME.typography.meta} text-txt-dim tracking-widest`}
                    >
                      // VOID
                    </span>
                  )}
                ...
                <div className="flex gap-8 md:gap-12 shrink-0">
                  <div className="flex flex-col gap-2 md:text-right">
                    ...
                    <span className={`${THEME.typography.meta} text-txt-muted`}>
                      {currentNode.lastModified || "2024.01.01"}
                    </span>
                  </div>
