...
export const HeaderTabs: React.FC = () => {
  const { navigate, currentNode, lang, toggleLang, theme, cycleTheme } = useNavigation();
  const scrollRef = useRef<HTMLDivElement>(null);
  const { canScrollLeft, canScrollRight, scroll, checkScroll } = useScrollOverflow(scrollRef);

  return (
    <header className={`${THEME.layout.barHeight} w-full bg-surface flex items-center relative shrink-0 z-50 group justify-between select-none border-b border-border`}>
      <div className="flex-1 overflow-hidden relative h-full flex items-center">
        ...
        {/* 
            OPTICAL COMPENSATION LOGIC:
            The buttons have 'px-3' (0.75rem) internal padding.
            To align the TEXT inside the button with the global grid line (--layout-gutter),
            we subtract 0.75rem from the container's padding.
            
            Result: The text starts exactly at var(--layout-gutter), 
            while the button hover background extends left into the gutter.
        */}
        <div 
            ref={scrollRef} 
            ...
            {QUICK_ACCESS.map((node) => {
                ...
                return (
                    <button
                        ...
                        // px-3 is exactly 0.75rem, matched with the calc() above
                        className={`${THEME.typography.ui} relative px-3 py-2 whitespace-nowrap rounded-sm group/tab ${isActive ? THEME.navigation.text.active : THEME.navigation.text.base}`}
                    >
                        <span className="relative z-10">{node.shortTitle?.[lang] || node.title[lang]}</span>
                        {!isActive && <span className="absolute inset-0 bg-overlay/5 opacity-0 group-hover/tab:opacity-100 transition-opacity rounded-sm" />}
                    </button>
                );
            })}
      ...
      <div className="flex items-center h-full pl-2 pr-4 bg-transparent z-40 mix-blend-screen gap-2">
          {ENABLE_THEME_SWITCHER && (
              <button onClick={cycleTheme} className={`flex items-center gap-2 px-2 py-1 ${THEME.typography.ui} ${THEME.navigation.text.base} hover:bg-overlay/5 rounded`} title={`Theme: ${theme.toUpperCase()}`}>
                 <Palette className="w-3 h-3" strokeWidth={1.5} />
                 <span className="hidden md:inline">{theme.toUpperCase()}</span>
              </button>
          )}

          <button onClick={toggleLang} className={`flex items-center gap-2 px-2 py-1 ${THEME.typography.ui} ${THEME.navigation.text.base} hover:bg-overlay/5 rounded`}>
