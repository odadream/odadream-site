...
export const HeaderTabs: React.FC = () => {
  ...
  return (
    <header className={`${THEME.layout.barHeight} w-full bg-surface flex items-center relative shrink-0 z-50 justify-between select-none border-b border-border`}>
      ...
      <div className="flex items-center h-full pl-2 pr-4 bg-transparent z-40 mix-blend-screen gap-2">
          {ENABLE_THEME_SWITCHER && (
              <button onClick={cycleTheme} className={`flex items-center gap-2 px-2 py-1 ${THEME.typography.ui} text-txt-muted hover:text-hud hover:bg-overlay/5 rounded transition-colors`} title={`Theme: ${theme.toUpperCase()}`}>
