---
id: collab
parent: home
title_en: Collaboration
title_ru: Сотрудничество
type: hub
tags: [handshake, network]
order: 4
visible: true
date: 2026.05.25
---

## COLLABORATION

Collaboration here is not a service — it is a form of co-presence.  
We adapt the work to space, audience, and story.

## Technical scope

**We do not deliver turnkey event production.** ODA.dream brings the neuro-art core; the venue or organizer provides infrastructure.

**From ODA.dream:** EEG neurointerfaces, laptop, proprietary software (Neuromandala and format modules), art works, facilitation, and on-site operators (depending on format).

**From the venue / organizer:** screen or TV (65″+, HDMI), projector if needed, microphones and sound output from the laptop, tables and chairs, power (typically 5 outlets, up to ~200 W load), HDMI cables, and the zone or stage footprint.

Format-specific requirements → [[lectures|Lectures]] (technical rider), [[neurobattle|Neurobattle]], [[mindshow|MindShow]]. Full PDF riders on request.

| Format | Venue provides | ODA.dream provides |
|--------|----------------|-------------------|
| Lecture / workshop | Screen 65″+, 2 mics, sound from PC, power, table, chair | 2 neurointerfaces, laptop, software, presenters |
| Neurobattle booth | 9 m²+ zone, screen 65″+, table, 2 chairs, 220 V | 2 neurointerfaces, laptop, software, 2 operators |
| MindShow booth | 6 m²+ zone, screen 65″+, table, chair, 220 V | Neurointerface, laptop, software, artist + host |

## Commercial track & expert appearances

**Paid commissions** (brand or private production) → [[registry-commercial|Commercial registry]] · [[for-business|Corporate & brands]]

**Invitations as experts** (forums, universities, festivals) → [[registry-expert|Expert registry]] · [[letters|Recognition & awards]]

## For Institutions & Collectors

[[for-institutions|Exhibition · Acquisition · Residency · Commission]]  
For gallery curators, museum programmers, contemporary art collectors, festival directors, and residency programs.

## For Education

[[for-education|Lectures · Workshops · Youth Programs · Summer Schools]]  
For universities, schools, and anyone building the next generation's relationship with science, art, and the inner life.

## For Corporate & Brands

[[for-business|A format that elevates your event]]  
For companies that invite only what is genuinely meaningful — and want their guests to know it.

## For Galleries & Art Spaces

[[for-galleries|Curatorial collaboration]]  
Site-specific installation, co-curated public programs, mediation, and production support.

## For Artists & Creative Teams

[[for-artists|Co-production and interdisciplinary labs]]  
Hybrid team projects where artistic concept, technology, and facilitation develop together.

## For Event Agencies

[[for-agents|Agency partnership]]  
You bring the client — we deliver the neuro stack, software, art layer, and on-site performance; the organizer provides AV, furniture, and power.

---

If you are not sure which section fits — write to us directly: [[contacts|contacts]]

---RU---

## СОТРУДНИЧЕСТВО

Сотрудничество здесь — не услуга, а форма соучастия.  
Мы адаптируем работу под пространство, аудиторию и историю.

## Технический контур

**Мы не делаем ивент-продакшн «под ключ».** ODA.dream привозит нейро-арт ядро; инфраструктуру обеспечивает площадка или организатор.

**От ODA.dream:** ЭЭГ-нейроинтерфейсы, ноутбук, авторское ПО («Нейромандала» и форматные модули), арт-работы, фасилитация и операторы на площадке (по формату).

**От площадки / организатора:** экран или TV (от 65″, HDMI), проектор при необходимости, микрофоны и звук с ноутбука, столы и стулья, электропитание (обычно 5 гнёзд, нагрузка до ~200 Вт), HDMI-кабели, зона или сцена.

Детали по форматам → [[lectures|Лекции]] (технический райдер), [[neurobattle|Нейробитва]], [[mindshow|Майнд-шоу]]. Полные PDF-райдеры — по запросу.

| Формат | Площадка | ODA.dream |
|--------|----------|-----------|
| Лекция / воркшоп | Экран 65″+, 2 мика, звук с ПК, розетки, стол, стул | 2 нейроинтерфейса, ноутбук, ПО, ведущие |
| Стенд «Нейробитва» | Зона от 9 м², экран 65″+, стол, 2 стула, 220 В | 2 нейроинтерфейса, ноутбук, ПО, 2 оператора |
| Стенд «Майнд-шоу» | Зона от 6 м², экран 65″+, стол, стул, 220 В | Нейроинтерфейс, ноутбук, ПО, артист и ведущий |

## Коммерция и экспертные приглашения

**Платные заказы** (бренд или частный продакшн) → [[registry-commercial|Реестр коммерции]] · [[for-business|Корпоративным клиентам]]

**Приглашения как эксперты** (форумы, вузы, фестивали) → [[registry-expert|Реестр экспертизы]] · [[letters|Признание и награды]]

## Институциям и коллекционерам

[[for-institutions|Выставка · Приобретение · Резидентура · Комиссионная работа]]  
Для кураторов галерей, музейных программистов, коллекционеров современного искусства, директоров фестивалей и программ резидентур.

## Образованию

[[for-education|Лекции · Воркшопы · Программы для молодёжи · Летние школы]]  
Для университетов, школ и всех, кто формирует отношение следующего поколения к науке, искусству и внутренней жизни.

## Корпоративным клиентам и брендам

[[for-business|Формат, который поднимает статус события]]  
Для компаний, которые приглашают только то, что по-настоящему значимо — и хотят, чтобы гости это почувствовали.

## Галереям и арт-пространствам

[[for-galleries|Кураторское сотрудничество]]  
Сайт-специфические инсталляции, совместные публичные программы, медиация и производственная поддержка.

## Художникам и креативным командам

[[for-artists|Копродакшн и междисциплинарные лаборатории]]  
Гибридные проекты, где художественная идея, технология и фасилитация развиваются вместе.

## Ивент-агентствам

[[for-agents|Партнёрство]]  
Вы приводите клиента — мы обеспечиваем нейро-контур, софт, арт-часть и работу на площадке; инфраструктуру обеспечивает организатор.

---

Не уверены, какой раздел подходит — напишите напрямую: [[contacts|контакты]]
