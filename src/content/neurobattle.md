---
id: neurobattle
parent: hub-performances
title_en: Neurobattle
title_ru: Парадокс Успеха
type: content
tags:
  - esports
  - brain
image: /images/nodes/neurobattle.webp
date: 2026.05.29
order: 1
status: production
kind: product
subkind: game
presented_at:
  - "[[event-dano-ekoniva-2025]]"
media:
  - nrbt-gameplay-v9
  - nrbt-dano
  - nrbt-veter
  - nrbt-tnt
aliases:
  - neurobattle
---

Two participants control a single virtual object in real time. In form — tug of war. In essence — **a duel of mental states**. The system reads brain rhythms (EEG) and translates them into generative graphics on the screen.

> Victory belongs not to the stronger, but to the more aware.

## How It Works

To gain the upper hand, depending on the battle mode, players must either **achieve deep relaxation** (increase alpha activity) or **intense concentration** (boost beta activity).

The system tracks relative changes in each player's state here and now. It's not about "who is smarter" or "whose brain is more powerful", but who can better regulate their state in the moment.

**The key paradox** of the format: success in a confrontation requires not emotional intensity, but the ability to manage it. The game is designed so that the outcome is inevitable, yet each player's path to victory is unique.

No training is required — participants learn the mechanics on the fly. This adds an extra layer of excitement: spectators watch closely, discuss previous players' strategies, and become actively immersed in the action.

## Why It's Engaging

- **Instant insight:** a direct and vivid way to sense how much you are in control of your inner balance right now.
- **Spectacular contradiction:** it is fascinating to watch the outwardly calmer opponent win. This generates buzz, emotion, and a dense energetic atmosphere around the zone.
- **Collective learning:** each round becomes a topic of discussion — what worked, how each player handled it, which states led to victory.
- **Technology as a partner:** interaction takes place between participants _through_ the system, which provides honest feedback on their mental performance.

## Technical Specifications

- **Format:** one-on-one duels or tournament bracket.
- **Round duration:** 90 seconds.
- **Throughput:** 5–8 pairs per 30 minutes per station; two parallel setups can increase the flow.
- **From ODA.dream:** 2 wireless neurointerfaces, laptop, charging station, proprietary software; watch video -> ![[media:nrbt-gameplay-v9]]; two operators (technician and host).
- **From the venue:** screen or projector 65″+, HDMI; table for laptop, 2 chairs for participants; power (typically 5 outlets, up to ~200 W); optional PA for host commentary and laptop audio.
- **Not turnkey** — see [[hub-collab|Collaboration hub]] for the full technical scope.

## Application Contexts

- **Corporate event:** not just hub-team building, but a mindfulness session wrapped in a thrilling competition. Who in the hub-team stays calm under pressure? How do they do it? What can be learned from them?
- **Public venue** (festival, exhibition): a live popular science demonstration turned into an exciting duel. Attracts audiences with a taste for tech-driven aesthetics.
- **Private event:** an element of intellectual performance and a focal point for thoughtful guests. A format people talk about and want to discuss.

**Bottom line:** a technologically sophisticated, visually compelling, and conceptually coherent format. Works for high throughput, draws a live audience, and ensures deep participant engagement.

## Format in motion

- ![[media:nrbt-dano]] — DANO Olympiad / EkoNiva stand;
- ![[media:nrbt-veter]] — Wind of Change charity gala;
- ![[media:nrbt-tnt]] — TNT "Ty kak Ya".

**Book for your event →** [[hub-business|Corporate & Brands]] · [[hub-event-agencies|Agencies]]

Ready to create a scenario for your event where technology, people, and audience function as a single organism? Contact us via [[hub-contacts]] to discuss the concept.

---RU---

Два участника управляют одним виртуальным объектом в реальном времени. По форме — перетягивание каната. По сути — **дуэль ментальных состояний**. Система считывает ритмы мозга (ЭЭГ) и транслирует их в генеративную графику на экране.

> Побеждает не тот, кто сильнее. Побеждает тот, кто осознаннее.

## Как это работает

Чтобы одержать верх, в зависимости от режима битвы нужно либо **максимально расслабиться** (рост альфа-активности), либо **максимально сконцентрироваться** (повышение бета-активности).

Система отслеживает относительные изменения состояний обоих игроков здесь и сейчас. Важно не то, «кто умнее» и не «чей мозг сильнее», а то, кто лучше управляет своим состоянием в моменте.

**Ключевой парадокс** формата: успех в противостоянии требует не накала эмоций, а умения ими управлять. Игра сконструирована так, что исход неизбежен, но способ победы у каждого игрока уникален.

Обучение не требуется — участники осваивают механику прямо в процессе. Это создаёт дополнительный драйв: зрители внимательно наблюдают, обсуждают стратегии предыдущих игроков и активно вовлекаются в происходящее.

## Почему это вовлекает

- **Мгновенный инсайт:** прямой и наглядный способ почувствовать, насколько вы владеете своим внутренним балансом прямо сейчас.
- **Зрелищное противоречие:** завораживает наблюдать, как выигрывает внешне более спокойный соперник. Это создаёт ажиотаж, эмоции и плотную энергетику вокруг зоны.
- **Коллективное обучение:** каждый раунд становится предметом обсуждения: что сработало, кто как справился, какие состояния привели к победе.
- **Технология как партнёр:** взаимодействие происходит между участниками _через систему_, которая даёт честную обратную связь о работе их психики.

## Техническая спецификация

- **Формат:** одиночные дуэли или турнирная сетка.
- **Длительность раунда:** 90 секунд.
- **Пропускная способность:** 5–8 пар за 30 минут на одну точку; возможна параллельная работа двух столов для увеличения потока.
- **От ODA.dream:** 2 беспроводных нейроинтерфейса, ноутбук, зарядная станция, авторское ПО; видео -> ![[media:nrbt-gameplay-v9]]; два оператора (техник и ведущий).
- **От площадки:** экран или проектор от 65″, HDMI; стол под ноутбук, 2 стула для участников; розетки (обычно 5 гнёзд, до ~200 Вт); при необходимости — звук для комментариев и вывода с ноутбука.
- **Не «под ключ»** — полный контур: [[hub-collab|хаб «Сотрудничество»]].

## Контексты применения

- **Корпоративный ивент:** не просто тимбилдинг, а сессия по осознанности в оболочке зрелищного состязания. Кто в команде сохраняет спокойствие под давлением? Как это получается? Чему у них можно поучиться?
- **Публичная площадка** (фестиваль, выставка): живая научно-популярная демонстрация, превращённая в увлекательный поединок. Притягивает аудиторию, ценящую технологичную эстетику.
- **Закрытое событие:** элемент интеллектуального перформанса и точка притяжения для вдумчивых гостей. Формат, о котором говорят и который хочется обсудить.

**Итог:** технологичный, зрелищный и концептуально цельный формат. Работает на поток, создаёт живую аудиторию вокруг и гарантирует глубокое вовлечение участников.

## Формат в движении

- ![[media:nrbt-dano]] — олимпиада DANO / стенд ЭкоНива;
- ![[media:nrbt-veter]] — благотворительный вечер «Ветер Перемен»;
- ![[media:nrbt-tnt]] — ТНТ «Ты как Я».

**Заказать на событие →** [[hub-business|Корпоративным клиентам]] · [[hub-event-agencies|Агентствам]]

Готовы спроектировать для вашего события ситуацию, где технология, человек и зритель работают как единое целое? Свяжитесь с нами через [[hub-contacts|контакты]], чтобы обсудить концепцию.
