---
id: hub-testimonials
parent: hub-world
title_en: Testimonials
title_ru: Отзывы
type: content
tags:
  - quote
  - speech
order: 3
date: 2026.05.27
---

## ECHOES

Verified words from organizers, institutions, and partners — about the duo and about specific hub-works. Each quote links back to the engagement where it was given; the full set is a single ledger in `data/hub-registry/proofs.yaml` (kind: testimonial).

<!-- hub-registry:testimonials-all -->
> «for co-presentation at TechFriendly Weekend, part of CIPR — Digital Industry of Industrial Russia forum»
>
> — CIPR (2026)

> «for participation in the first AI festival "Cultural Code of Russia", interest in the project, and creation of your work»
>
> — Tavrida.ART (2025)

> «for conducting an immersive lecture "Beautiful About the Brain" that united neurophysiology, EEG technologies, and science-art»
>
> — HSE University (2025)

> «for the birthday celebration of Ashram studio — a neuro-art experience that resonated deeply with our wellness community»
>
> — Ashram Studio (Natali Osman) (2024)

> «for live neuro-mandala visualization at the 3rd International Meditation Day»
>
> — Organizing committee, 3rd International Meditation Day (2026)

> «for the art installation "Neuromandala" that united art, science, and technology»
>
> — Organizing committee of the III forum "Bitva za Milliard" (2024)
<!-- /hub-registry:testimonials-all -->

---RU---

## ЭХО

Подтверждённые формулировки организаторов, институций и партнёров — о дуэте и о конкретных работах. Каждая цитата связана с участием, где она прозвучала; полный корпус — единым реестром в `data/hub-registry/proofs.yaml` (kind: testimonial).

<!-- hub-registry:testimonials-all-ru -->
> «за совместное выступление на TechFriendly Weekend в рамках форума ЦИПР — Цифровая индустрия промышленной России»
>
> — ЦИПР (2026)

> «за участие в первом фестивале ИИ «Культурный код России», интерес к проекту и создание своей работы»
>
> — Таврида.АРТ (2025)

> «за проведение иммерсивной лекции «Красиво про мозг», объединившей нейрофизиологию, ЭЭГ-технологии и science-art»
>
> — НИУ ВШЭ (2025)

> «за нейро-арт-опыт на праздновании дня рождения студии Ашрам, который глубоко отозвался в нашем велнес-сообществе»
>
> — Студия Ashram (Натали Осман) (2024)

> «за живую визуализацию нейромандалы на III Международном дне медитации»
>
> — Оргкомитет III Международного дня медитации (2026)

> «за арт-инсталляцию «Нейромандала», объединившую искусство, науку и технологии»
>
> — Оргкомитет III форума «Битва за миллиард» (2024)
<!-- /hub-registry:testimonials-all-ru -->
