...
const CHANGELOG_PATH = path.join(PROJECT_ROOT, 'src', 'content', 'changelog.md');
...
const metaDesc = history.slice(0, 3).map(h => {
...
const badgeRegex = /!\[Version\]\(https:\/\/img\.shields\.io\/badge\/version-.*-emerald\)/;
...
const changelogEn = history.map(h => {
    return `
...
const changelogRu = history.map(h => {
    return `
